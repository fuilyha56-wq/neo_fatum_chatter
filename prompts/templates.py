...
提示词内容移植自老版 NeoFatumChatter 的丰富行为引导，
...
# ─── 回复方式：JSON 模式 ─────────────────────────────────────────
NFC_REPLY_MODE_JSON = """每次做出决策时，必须在消息开头输出一个 JSON 对象作为你的决策内容：
...
# ─── 回复方式：完全工具调用模式 ────────────────────────────────
NFC_REPLY_MODE_TOOL_CALLING = """你的所有决策必须通过工具调用来执行：
...
NFC_PERCEIVE_FOLLOWUP_PROMPT_JSON = """<perception_completed>
你已经看过了上面的未发送草稿。
...
# ─── 主动发起决策指令（按模式区分） ──────────────────────────
NFC_PROACTIVE_DECISION_JSON = """如果你产生了真实的表达欲，请通过输出 JSON 对象来执行你的决策（content 参数不能为空）。
