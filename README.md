...
- 🫀 **内驱状态机**（v2.6.5 新）：社交欲/精力/被忽视感等随时间连续演化，bot 在两次消息之间也在"过日子"，回复温度与等待时长被真实状态调制 → 详见 [MECHANISM.md](MECHANISM.md)
- 👁️ **S1 感知评估**（v2.6.5 新）：每批消息先由轻量模型产出结构化第一反应（情绪/紧急度/场景事实/话题钩子/承诺），并建议"缓几秒再回"的自然延迟
- 🎭 **双登记簿世界**（v2.6.5 新）：拟人聊天与剧情 RP 同引擎——现实/故事两个世界状态机，支持入戏协商、现实急事暂停、故事存档续写、跨世界记忆渗透
- 💎 **信念记忆**（v2.6.5 新）：压缩后蒸馏"对 TA 的持久理解"（带置信度、可被新证据推翻），主动追问有内容有理由
- ⏳ **自然节奏**：等待 / 超时追问 / 沉默 / 语义化打字延迟（按字数与标点）/ 分段发送（可选流式打字机）
- 💬 **主动联系**：意图队列（承诺到期/话题钩子/内驱溢出）+ 预约 + 沉默兜底 + 勿扰时段
- 📦 **消息积累**：连发消息合并处理，打断冷却递增 + 连续上限防刷屏
- 🖼️ **原生多模态**：图片直接进 LLM 上下文，可跳过 VLM 转述
- 🧩 **用户画像**：习惯记录 / 查询 / 纠正 / 删除 + 活跃时段学习
