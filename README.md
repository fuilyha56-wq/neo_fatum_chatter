...
> ## v2.5.4 更新
>
> - **enabled 开关一键让位**：`[general].enabled=false` 时动态注销 Chatter 并重启受影响流循环，DFC 等可立即接管，无需重启。
> - **注入点可配置**：新增 `[flashback].injection_point`（默认 `default_chatter_user_prompt`，对齐 booku_memory / DFC bridge）。
> - **合并 v2.5.3 三项修复**：打断消息装哑 / tool_call 续轮 assistant 丢失 / 主动思考炸缓存。
>
> ---
>
> **版本选择说明**
...
> | 版本 | 说明 |
> |------|------|
> | `v2.5.4` | **当前推荐** — enabled 动态切换 + 注入点可配置 + v2.5.3 三项修复 |
