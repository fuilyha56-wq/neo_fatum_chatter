...
> | `v2.3.2` | **当前默认推荐** | v2.3 正式版：感知兜底回填改用 sub actor 提取、`perception_extract_task` 可配置、上下文污染修复、NFC Action 注册兼容修复 |
>
> **维护说明**：当前仅 `v2.3.2` 处于主线维护。`v2.0.2` / `v2.1.1` / `v2.2.2-beta` / `v2.3.0-beta` / `v2.3.0-beta.1` / `v2.3.1` 仅作为历史参考与回退选项保留，**不再接受 bug 修复或兼容性更新**；如遇问题请优先升级到当前默认推荐版本。
