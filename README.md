...
>
> **新功能**
> - 情绪轨迹追踪：自动记录每次决策的情绪，可查询近期情绪走势
> - 用户活跃时段学习：按小时统计用户活跃规律，主动发起时参考活跃时段
> - 多段消息段间延迟：分段回复之间加入随机间隔，模拟真人节奏（可配置）
> - 对话中断恢复：进程重启后自动检测未处理消息并恢复对话
> - 流式回复配置预留（默认关闭，待框架支持消息编辑后启用）
>
> **优化**
> - orchestrator 异常处理合并，减少约 80 行重复代码
> - session 索引文件原子写入，防止写入中途崩溃导致损坏
> - chain_payloads 反序列化增加类型校验
> - 压缩器消息上限保护（超 5000 条自动截断）
> - 主动发起概率衰减：沉默越久触发概率越高
> - 调度器等待改为指数退避，首次注册更快
> - 删除 ast.literal_eval 依赖和 reply_json 死代码
