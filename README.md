...
> **版本选择说明（重要，置顶）**
>
> 当前 GitHub Release 保留多个可下载版本；插件市场默认安装最新版本。若新版本与本地环境、模型供应商或其他插件协作方式不合适，可从 GitHub Release 手动下载旧版 `.mfp`。
>
> | 版本 | 定位 | 优点 | 缺点 / 注意 |
> |------|------|------|-------------|
> | `v2.0.1` | 历史稳定版 | 最保守，适合需要完全维持旧行为的环境 | 不包含后续 manifest 身份修复、系统提示词空值修复、作者信息更新 |
> | `v2.0.2` | baseline 修复版 | 保留原上下文行为，同时包含 manifest 身份修复、系统提示词空值修复、作者信息更新；适合担心 prompt cache 优化影响行为的人 | 没有进一步强化 LLM prompt prefix cache 命中，厂商缓存折扣收益较低 |
> | `v2.0.3` | 缓存命中优化版（默认推荐） | 在保持对外接口不变的前提下优化 prefix cache：去掉相对分钟数、降低无历史时间精度、冻结融合叙事；更容易吃到 DeepSeek / Claude / Qwen / OpenAI 等厂商的 prompt cache 折扣 | 首次生成冻结叙事时会多保存一次 session；历史叙事在同一 `chain_cutoff_ts` 下更稳定，极少数依赖“每次重建都重扫 MentalLog 细微变化”的场景可能感知差异 |
>
> 兼容性说明：`v2.0.3` 不修改组件名称、Action 名称、配置 schema 或 manifest include；新增的 session 字段使用软读取，`v2.0.1` / `v2.0.2` / `v2.0.3` 之间切换不会破坏已有 session 数据。
...
运行时主流程也已从 `chatter.py` 中抽离到 `runtime/orchestrator.py`，`KokoroFlowChatter` 现在主要保留门面与 helper。回合准备与提交已进一步下沉到 `runtime/turn_controller.py`，消息积累窗口与 LLM 打断控制分别收敛在 `runtime/message_buffer.py`、`runtime/interrupt_controller.py`。超时、主动预约、近期摘要、多模态历史图片注入等外围副作用则通过 `services/` 目录进入主流程。

其中：

- protocol/compat_adapter.py 处理 provider 兼容与 DeepSeek tool-call 补正
- protocol/response_normalizer.py 负责把响应统一收敛为标准视图
- protocol/decision_parser.py 把执行结果统一收敛为 Decision

框架层仍通过原生 Tool Calling 驱动对话，核心动作如下：
...
│   └── scene_state.py       # 显式场景状态与证据模型
├── runtime/
│   ├── orchestrator.py      # execute 主循环编排
│   ├── turn_controller.py   # 回合准备与决策提交
│   ├── interrupt_controller.py # LLM 打断控制
│   └── message_buffer.py    # 连发消息积累窗口
├── context/
│   ├── planner.py           # 上下文规划门面
│   ├── renderer.py          # 上下文渲染门面
│   ├── types.py             # ContextPlan / ContextContribution / StatePatch
│   └── sources/
│       ├── initial_source.py # 初始上下文的 session/config 规划
│       ├── history_source.py # history summary / chain / narrative source
│       ├── scene_source.py   # SceneState -> system prompt 结构化渲染
│       └── plugin_source.py  # 第三方上下文兼容接入点
├── protocol/
│   ├── compat_adapter.py    # Provider 兼容适配
│   ├── response_normalizer.py # 响应标准化
│   └── decision_parser.py   # ToolResult -> Decision
├── session.py               # 会话状态持久化
...
│   ├── builder.py           # 提示词门面（委托到 context/）
│   └── modules.py           # 模板注册与上下文构建
├── services/
│   ├── multimodal_service.py # 多模态历史图注入门面
│   ├── proactive_service.py  # schedule_proactive session 副作用
│   ├── summary_service.py    # 近期摘要压缩调度
│   └── timeout_service.py    # 超时 payload 与工具链闭合
├── debug/
...
| `max_compat_retries` | `1` | 纯文本感知草稿转工具调用的最大重试次数 |
