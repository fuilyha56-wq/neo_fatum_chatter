...
这份文档用于把 NeoFatumChatter 的重构任务交接给下一个 agent。

目标不是继续在现有结构上打补丁，而是把 NFC 从“单文件臃肿总控 + 多条隐式注入链 + 多套决策协议并存”的状态，重构为一个边界清晰、可维护、可继续扩展的私聊关系驱动聊天器。
...
NFC 有关系感、等待感、生活连续性、主动联系、近期记忆，但没有明确的 `SceneState`。
...
这意味着 NFC 实际在维护多套“模型输出 -> 插件内部动作”的转换协议。
...
- `nfc_system_prompt` 的模板构建事件
- `nfc_user_prompt` 的手工 on_prompt_build 事件
- actor reminder
...
- NFC 唯一正式决策入口为：`tool calling -> normalize -> Decision`
...
- NFC 必须能区分 `self_state`、`user_state`、`relationship_state`、`scene_evidence`、`notice`
...
1. `policy`
协议、硬约束、格式要求；默认只允许 core / NFC 自己写入。
...
否则第三方工具只要返回一段强指令文本，就等于绕过了 NFC 的上下文和权限模型。
...
- tool_result 链闭合正确
- 缺失 call_id 时本地补齐
- NFC system prompt 不应把通道参数渲染成具体设备场景
...
- 明确 NFC 的唯一正式协议为原生 tool calling
...
“内心活动流”是 NFC 的识别性能力，问题不在它存在，而在它与历史、摘要、场景的耦合方式。
...
这是 NFC 非常实用的能力，也应当保留，但要从 `chatter.py` 独立出去。
...
NFC 现在的问题不是“功能太多”，而是“同一份功能被塞进了错误的边界”。
